...
class CustomerController extends Controller
{
    ...
    // PUT /api/customers/{id}/addresses/{aid} — Update Shipping Address
    public function updateAddress(Request $request, $id, $aid): JsonResponse
    {
        $customer = Customer::findOrFail($id);
        $address = ShippingAddress::where('customer_id', $customer->id)->findOrFail($aid);
        ...
        try {
            DB::transaction(function () use ($request, $customer, $address) {
                ...
                ActivityLog::create([
                    'user_id' => auth()->id(),
                    'action' => 'อัปเดต ShippingAddress',
                    'description' => "อัปเดตที่อยู่จัดส่ง '{$address->label}' สำหรับลูกค้า {$customer->name}",
                    'entity_type' => 'customers',
                    'entity_id' => $customer->id,
                ]);
            });
    ...
    // DELETE /api/customers/{id}/addresses/{aid} — Delete Shipping Address
    public function destroyAddress($id, $aid): JsonResponse
    {
        $customer = Customer::findOrFail($id);
        $address = ShippingAddress::where('customer_id', $customer->id)->findOrFail($aid);
        ...
        try {
            DB::transaction(function () use ($customer, $address) {
                ...
                ActivityLog::create([
                    'user_id' => auth()->id(),
                    'action' => 'ลบ ShippingAddress',
                    'description' => "ลบที่อยู่จัดส่ง '{$address->label}' ของลูกค้า {$customer->name}",
                    'entity_type' => 'customers',
                    'entity_id' => $customer->id,
                ]);
            });
